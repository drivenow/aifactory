...
@tool("nonfactor_list")
def nonfactor_list(lang: str = "py") -> Dict[str, List[Dict[str, str]]]:
    """
    返回可用 nonfactor 列表。
    Args:
        lang: 'py' or 'cpp'，默认 'py'
    """
    meta_map = PY_NONFACTOR_META if lang == "py" else CPP_NONFACTOR_META
    factors = []
    for name, meta in meta_map.items():
        factors.append({"name": name, "desc": meta.desc, "sample_rate": "1s"})
    return {"factors": factors}


@tool("nonfactor_source")
def nonfactor_source(name: str, lang: str = "py") -> Dict:
    """
    获取指定 nonfactor 的源码与字段说明。
    Args:
        name: nonfactor 名称
        lang: 'py' or 'cpp'，默认 'py'
    """
    meta_map = PY_NONFACTOR_META if lang == "py" else CPP_NONFACTOR_META
    paths_map = PY_NONFACTOR_PATHS if lang == "py" else CPP_NONFACTOR_PATHS

    if name not in paths_map:
        return {"ok": False, "error": f"Non-factor '{name}' not found."}

    file_path = paths_map[name]
    try:
    ...
    return {"ok": True, "source": read_res.get("content"), "meta": meta_dict}
