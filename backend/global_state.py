...
class FactorAgentState(
    ...
    state_error: Annotated[List[str], keep_last] #把 error 改成可聚合通道， langgraph会合并状态
    ...
    eval_type: Optional[str] = "mock"         # 评价类型：mock/l3/alpha
...
class FactorAgentStateModel(BaseModel):
    ...
    # RetryState
    retry_count: int = 0
    state_error: Optional[List[str]] = Field(default_factory=list)
